...
function App() {
  ...
  return (
    <main
      className={`w-full min-h-screen bg-[url('weather-bg.jpg')] bg-cover bg-center bg-no-repeat bg-fixed flex flex-col justify-center items-center px-4 py-8 overflow-y-auto ${active && "gap-6"}`}
    >
      <div className="w-full flex flex-col justify-center items-center gap-2">
        ...
        {!showingBar ? (
          ...
          <SearchBar className="w-4/5 xs:w-3/5 md:w-3/7" onKeyDown={(e) => e.key === "Enter" && handleSearch(e)} />
        )}
      </div>

      {active && data && (
        <Forecast className="shadow-md shadow-black grid grid-cols-1 gap-3 rounded-lg p-3 xs:grid-cols-2 md:grid-cols-3">

          <Card className={`bg-linear-to-b ${theme.gradient} xs:col-span-2 md:col-span-1 md:order-1`}>
            <SearchBtn
          ...
          <Card className="md:order-0">
            <img src={theme.img} alt="Weather Icon" className="w-28 h-28 xs:w-20 xs:h-20 sm:w-26 sm:h-26" />
            <p className="text-base text-slate-700">
